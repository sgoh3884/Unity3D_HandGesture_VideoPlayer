...
#pragma warning restore 0649
		...
		#region Constants
		private const string ALL_FILES_FILTER_TEXT = "Video Files";
...
#pragma warning restore 0649
			...
			if( pathChanged )
			{
				...
				if( !string.IsNullOrEmpty( m_currentPath ) )
				{
					try
					{
                        ...
                        string[] extension = { ".AVI", ".avi", ".MP4", "mp4", ".MKV", ".mkv", ".WMV", ".wmv", ".MOV", ".mov", ".TS", ".ts", ".TP", ".tp" };
                        for (int i = 0; i < files.Length; i++)
                        {
                            ...
                            {
                                if (file.Contains(extension[j]))
                                {
                                    allItems.Add(files[i]);
                                    break;
                                }
                            }
                        }
		...
		private bool AddQuickLink( Sprite icon, string name, string path, ref Vector2 anchoredPos )
		{
            ...
            cube.transform.parent = quickLink.transform;
            cube.GetComponent<BoxCollider>().isTrigger = true;
            cube.AddComponent<FileHover>();

            RectTransform rt = cube.gameObject.AddComponent<RectTransform>();
            rt.localPosition = new Vector3(0, -15, 0);
            rt.localScale = new Vector3(150, 20, 5);
